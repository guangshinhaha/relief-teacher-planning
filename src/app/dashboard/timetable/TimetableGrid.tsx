...
import { addTimetableEntry, deleteTimetableEntry } from "./actions";
...
export default function TimetableGrid({
  ...
  function handleTeacherChange(teacherId: string) {
    startTransition(() => {
      router.push(`/dashboard/timetable?teacherId=${teacherId}`);
    });
  }
  ...
  async function handleSave() {
    ...
    formData.set("dayOfWeek", editingCell.dayOfWeek.toString());
    formData.set("periodId", editingCell.periodId);
    formData.set("className", formClassName);
  ...
  if (periods.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded-xl border border-card-border bg-card px-6 py-16 text-center shadow-sm">
        ...
        <p className="mt-1 text-xs text-muted">
          Set up periods first from the Settings page.
        </p>
      </div>
    );
  }
      ...
      <div className="overflow-hidden rounded-xl border border-card-border bg-card shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full border-collapse">
            <thead>
              <tr>
                <th className="w-[120px] border-b border-r border-card-border bg-background px-4 py-3 text-left text-xs font-medium uppercase tracking-wide text-muted">
                  Period
                </th>
